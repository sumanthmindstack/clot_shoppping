...
import '../../../../bloc/get_swp_data.dart/get_swp_data_cubit.dart';
import '../widget_shimmers/fund_card_shimmer.dart';
import 'widgets/fund_card_widget.dart';

class SwpTab extends StatefulWidget {
  final int userId;
  const SwpTab({super.key, required this.userId});

  @override
  State<SwpTab> createState() => _SwpTabState();
}

class _SwpTabState extends State<SwpTab> {
  int currentPage = 1;
  final int limit = 10;
  ...
  Widget build(BuildContext context) {
    return BlocConsumer<GetSwpDataCubit, GetSwpDataState>(
      listener: (context, state) {},
      builder: (context, state) {
        if (state is GetSwpDataLoadingState) {
          return const FundCardShimmerList();
        }

        if (state is GetSwpDataSuccessState) {
          return Column(
            children: [
  ...
  Widget _filterDataContainer(BuildContext context) {
    return CustomFilterSelectionWidget(
      onOptionSelected: (selectedOption) {
        context.read<GetSwpDataCubit>().fetchSwpData(
      ...
      filterConfigs: DropdownConstants().traansactionSwpFilterDropdowns,
      backgroundColor: AppColors.primaryColor,
    );
  }

  Widget _buildSwpList(GetSwpDataSuccessState state) {
    final data = state.getSwpDataEntity.data;
    final totalPages = state.getSwpDataEntity.meta!.totalPages;
    final pagination = _buildPagination(totalPages ?? 0);
