...
import 'package:maxwealth_distributor_app/features/investers/presentation/bloc/get_switch_data/get_switch_data_state.dart';

import '../../../../../../../constants/dropdown_constants.dart';
import '../../../../../../../themes/app_colors.dart';
import '../../../../../../../widgets/customfilter_selection_widget.dart';
import '../../../../bloc/get_switch_data/get_switch_data_cubit.dart';
import '../widget_shimmers/fund_card_shimmer.dart';
import 'widgets/fund_card_widget.dart';

class SwitchTab extends StatefulWidget {
  final int userId;
  const SwitchTab({super.key, required this.userId});
...
class _SwitchTabState extends State<SwitchTab> {
  ...
  Widget build(BuildContext context) {
    return BlocConsumer<GetSwitchDataCubit, GetSwitchDataState>(
      listener: (context, state) {},
      builder: (context, state) {
        if (state is GetSwitchDataLoadingState) {
          return const FundCardShimmerList();
        }

        if (state is GetSwitchDataSuccessState) {
          return Column(
            children: [
              _filterDataContainer(context),
              _buildSwitchList(state),
            ],
          );
        }
  ...
  Widget _filterDataContainer(BuildContext context) {
    return CustomFilterSelectionWidget(
      onOptionSelected: (selectedOption) {
        context.read<GetSwitchDataCubit>().fetchSwitchData(
      ...
      filterConfigs: DropdownConstants().traansactionSwitchFilterDropdowns,
      backgroundColor: AppColors.primaryColor,
    );
  }

  Widget _buildSwitchList(GetSwitchDataSuccessState state) {
    final data = state.switchDataEntity.data;
    final totalPages = state.switchDataEntity.meta!.totalPages;
    ...
    return Column(
      children: [
        ...data.map((item) => FundCard(
              transactionBasketItemId: item.transactionBasketItemId,
              amount: item.amount,
              fundName: item.switchOutFundName,
              scheme: item.switchInFundName,
              state: item.state,
            )),
