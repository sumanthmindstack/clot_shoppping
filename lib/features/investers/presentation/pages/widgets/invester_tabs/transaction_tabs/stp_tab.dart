...
import '../../../../bloc/get_stp_data.dart/get_stp_data_cubit.dart';
import '../widget_shimmers/fund_card_shimmer.dart';
import 'widgets/fund_card_widget.dart';

class StpTab extends StatefulWidget {
  final int userId;
  const StpTab({super.key, required this.userId});

  @override
  State<StpTab> createState() => _StpTabState();
}

class _StpTabState extends State<StpTab> {
  int currentPage = 1;
  final int limit = 10;
  ...
  Widget build(BuildContext context) {
    return BlocConsumer<GetStpDataCubit, GetStpDataState>(
      listener: (context, state) {},
      builder: (context, state) {
        if (state is GetStpDataLoadingState) {
          return const FundCardShimmerList();
        }

        if (state is GetStpDataSuccessState) {
          return Column(
            children: [
  ...
  Widget _filterDataContainer(BuildContext context) {
    return CustomFilterSelectionWidget(
      onOptionSelected: (selectedOption) {
        context.read<GetStpDataCubit>().fetchStpData(
      ...
      filterConfigs: DropdownConstants().traansactionStpFilterDropdowns,
      backgroundColor: AppColors.primaryColor,
    );
  }

  Widget _buildStpList(GetStpDataSuccessState state) {
    final data = state.getStpDataEntity.data;
    final totalPages = state.getStpDataEntity.meta!.totalPages;
